...
TouchScreen::TouchScreen() {
    _touch = new XPT2046_Touchscreen(T_CS/*, T_IRQ*/);
    _screen = new ILI9341_t3(TFT_CS, TFT_DC, TFT_RST, TFT_MOSI, TFT_SCK, TFT_SDO);
}
...
bool TouchScreen::read() {
    ...
    if(/*_touch->tirqTouched() &&*/ _touch->touched()) {
        ...
        TS_Point touchPoint{_touch->getPoint()};
        _x = map(touchPoint.x, 0, 4095, 0, _width);
        _y = map(touchPoint.y, 0, 4095, 0, _height);
